	...
	for(var i=0; i<numofActions; i++){
		...
		/////////////////////////////////
			...
			var string=Actions[i][0].toLowerCase().removeWS(), // CACHE FOR STRING THAT SPACES DON'T MATTER
				...
				// KRW change
				auto_convo=property.indexOf('auto_convo'),
			...
			if(auto_convo >=0) { window.console.log("auto_convo:"+value);
			//if(auto_convo >=0) $Char.talk_to(JAG, $Item, value, inInv);
//(JAG, $Item, iD, inInv){
	window.console.log(JSON.stringify(JAG));
	window.console.log(JSON.stringify($Item));
	window.console.log(JSON.stringify(iD)); }
	//window.console.log(JSON.stringify(inInv)); }						
